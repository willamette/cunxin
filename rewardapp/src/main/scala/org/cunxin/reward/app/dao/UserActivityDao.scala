...
class UserActivityDao @Inject()(db: MongoDB) extends AbstractMongoDao[UserActivity](db, "useractivity", version = 1) with MongoRWDao[UserActivity] {
    ...
    ensureIndex("projectId" -> 1, "idx_projectId", unique = true)
    ...
    def findUserActivitiesByPeriod(userId: String): List[DBManagedModel[UserActivity]] = findUserActivitiesByPeriod(userId, new Date(70, 0, 1), new Date())
    ...
    def findUserActivitiesByPeriod(userId: String, projectId: String): List[DBManagedModel[UserActivity]] = findUserActivitiesByPeriod(userId, projectId, new Date(70, 0, 1), new Date())
}
